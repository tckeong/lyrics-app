...
impl LyricsAPI {
    pub fn new() -> Self {
        Self {
            url: Utils::new().get_env("NETEASE_LYRICS_API_URL").unwrap(),
    ...
    fn get_valid_id(self) -> Result<Self, String> {
        ...
        // Map the songs to a tuple of (title, id, artist)
        ...
        // If found songs, return the first one from the filtered result
        // If no songs found, return the first song from the result
