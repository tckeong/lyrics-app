...
function AuthButton() {
    ...
    const className = "bg-gray-300 border-2 border-slate-500 rounded-lg text-md mx-2 py-2 px-2 font-mono hover:text-blue-700";
    ...
    const handleAuth = async () => {
        await invoke("auth_check").then((data) => {
            let [client_id , client_secret] = data as string[];
            return [client_id, client_secret];
        }).then(async ([client_id, client_secret]) => {
            invoke("login", { client_id: client_id, client_secret: client_secret })
                .then(async (auth_url) => {
